...
class ilObjBigBlueButtonGUI extends ilObjectPluginGUI
{
	...
	function showContent()
	{
		...
		$BBBHelper=new ilBigBlueButtonProtocol($this->object->getSvrPublicURL(), $this->object->getSvrSalt());
		...
		if($isModerator){
			...
			if ($all_recordings){
				...
				foreach($all_recordings as $recording){
					...
					// new Code
					$table_row_template->setVariable("Date",date("d.m.Y H:i",  substr ($recording->getStartTime(),0,10)));
					$table_row_template->setVariable("Length",$recording->getPlaybackLength());
					$table_row_template->setVariable("Link",$recording->getPlaybackUrl());
					$table_row_template->setVariable("DeleteLink", $recording->getRecordId);
